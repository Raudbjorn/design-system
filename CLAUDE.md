...
pnpm test                        # Vitest: unit (jsdom) + astro projects together
pnpm run test:watch              # same two projects, watch mode
...
Run a single unit test: `pnpm exec vitest run --project=unit src/lib/components/atoms/Button.test.ts` (or `-t "name"` to filter by test title). `--project=astro` runs only the Astro parity/barrel tests, kept in a separate project (needs Astro's own Vite plugin) so a broken `.astro` file can't take down the fast jsdom suite.

The Rust TUI (`crates/raudbjorn-tui`, Rust 1.96.1) has its own command family, not covered above: `pnpm run tui:check` / `tui:test` (cargo check/test), `pnpm run tui:gallery` / `tui:list` (interactive story browser), `pnpm run tui:smoke` (determinism check). See `docs/tui-gallery.md`.
...
**Native TUI.** `crates/raudbjorn-tui` is a parallel Rust/Ratatui rendering of the same component set — a development/verification surface, not an npm export. `templates/*.crepus` holds the production terminal templates (don't duplicate them as imperative Ratatui widgets); `src/catalog` is the typed story inventory (story IDs are stable snapshot keys); `src/theme/generated.rs` is generated by `scripts/emitters/emit-tui-rust.mjs` from the same resolved DTCG palettes as the web/QSS outputs — never hand-edit it. Ratatui `Buffer` snapshots are the automated oracle; `src/lib/index.ts`'s catalog-coverage test fails on any web component left unclassified in the Rust catalog. See `docs/tui-gallery.md`.
