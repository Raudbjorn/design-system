...
`@svnbjrn/design` bundles and redistributes the following fonts. The project's
own source code is MIT-licensed (see `LICENSE`), but these font files — the
originals under `assets/fonts-src/` and the subset/converted `.woff2` files
under `src/lib/fonts/` and generated into `dist/fonts/` — are licensed under
the **SIL Open Font License, Version 1.1**, and remain so after subsetting and
format conversion.
...
- **Iosevka** (Nerd Font patched) — Copyright (c) 2015-2026 Belleve Invis
  (https://github.com/be5invis/Iosevka). Nerd Fonts glyph additions Copyright
  (c) Ryan L McIntyre (https://github.com/ryanoasis/nerd-fonts). Licensed under
  SIL OFL 1.1.

The `.woff2` files shipped by this package are subsets of the above, produced
